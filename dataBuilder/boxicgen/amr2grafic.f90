program amr2grafic
  !--------------------------------------------------------------------------
  ! altered amr2cube to output all hydro variables to grafic
  ! takes into account boxlen in header 
  ! Version Tine Colman 07/01/20
  ! Version Tine Colman 18/05/17
  ! Version F90 par R. Teyssier le 01/04/01.
  !--------------------------------------------------------------------------
  implicit none
  integer::ndim,n,i,j,k,twotondim,ncoarse,type=0,domax=0
  integer::ivar,ifield,nvar,ncpu,ncpuh,lmax=0,nboundary,ngrid_current
  ...
  character(LEN=128)::nomfich,repository,outfich,filetype='bin',solver='hydro'
  ...
  do ilevel=1,lmax
     ...
     allocate(grid(ilevel)%cube(imin:imax,jmin:jmax,kmin:kmax,nvarh))
     ...
     do ilevel=1,lmax
        ...
        if(ngrida>0)then
           ...
           do ind=1,twotondim
              ...
              ! Extract variable and store data cube
              do i=1,ngrida
                 ok_cell= .not.ref(i)
                 if(ok_cell)then
                    ...
                    if(    ix>=grid(ilevel)%imin.and.&
                       ...
                       do ifield=1,nvarh
                          grid(ilevel)%cube(ix,iy,iz,ifield)=var(i,ind,ifield)
                       end do
                    endif
                 end if
              end do
        ...
        do iy=jmin,jmax
           ...
           do iz=kmin,kmax
              ...
              do ilevel=max(levelmin,lmin),lmax-1
                 ...
                 j=int(ymin*ndom)+1
                 k=int(zmin*ndom)+1
                 grid(lmax)%cube(ix,iy,iz,ifield)=grid(lmax)%cube(ix,iy,iz,ifield)+ &
                   & grid(ilevel)%cube(i,j,k,ifield)
              end do
           end do
        end do
     end do
     write(*,*)'Min value:',minval(grid(lmax)%cube(imin:imax,jmin:jmax,kmin:kmax,ifield))
     write(*,*)'Max value:',maxval(grid(lmax)%cube(imin:imax,jmin:jmax,kmin:kmax,ifield))
     write(*,*)'Norm:     ',sum   (dble(grid(lmax)%cube(imin:imax,jmin:jmax,kmin:kmax,ifield))) &
          & /(imax-imin+1)/(jmax-jmin+1)/(kmax-kmin+1)
  end do

  ! Output file 
  do ifield=1,nvarh
     if(solver.eq.'hydro') then
        select case (ifield)
        case (1)
           varname='d' 
        case (2)
        ...
        case (5)
           varname='p'
        case default
           call title(ifield-5,ncharcpu)
           varname='pvar_'//TRIM(ncharcpu)
        end select
     else
        select case (ifield)
        case (1)
        ...
        case (11)
           varname='p'
        case default
           call title(ifield-11,ncharcpu)
           varname='pvar_'//TRIM(ncharcpu)
        end select
     end if

     nomfich=TRIM(outfich)//'/ic_'//TRIM(varname)
     write(*,*)'Writing file '//TRIM(nomfich)
     open(unit=20+ifield,file=nomfich,form='unformatted')
     dummy=0.0
     write(20+ifield)imax-imin+1,jmax-jmin+1,kmax-kmin+1,boxlen/(2**lmax),dummy,dummy,dummy,boxlen,dummy,dummy,dummy
     do iz=kmin,kmax
        write(20+ifield)((real(grid(lmax)%cube(ix,iy,iz,ifield)),ix=imin,imax),iy=jmin,jmax)
     end do
     close(20+ifield)
  end do
  ...
  subroutine read_params
    ...
    if (n < 4) then
       ...
       print *, 'ex: amr2cube -inp output_00001 -out cube.dat'// &
            &   ' -siz 0.25 -xmi 0.1 -xma 0.7 -lma 12'
       stop
    end if
